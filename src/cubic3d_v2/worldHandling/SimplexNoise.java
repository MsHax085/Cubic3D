...
public class SimplexNoise {  // Simplex noise in 2D, 3D and 4D
  ...
  private double noise2D(double xin, double yin) {
    ...
    public float compute2D(float x, float y) {
    ...
    public float compute3D(float x, float y, float z) {
        float frequency = 0.033f;
        float amplitude = 1.2f;
        float persistence = 0.12f;
        int octaves = 4;
        ...
        for (int i = 0; i < octaves; i++) {
            noise += noise3D(x * freq, y * freq, z * freq) * amp;
            freq *= 2;
            amp *= persistence;
        }
        return noise;
